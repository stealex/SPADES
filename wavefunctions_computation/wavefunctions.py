...
from configs.wavefunctions_config import atomic_system

class wavefunctions_handler:
    def __init__(self, atom:atomic_system, bound_conf=None, scattering_conf=None) -> None:
        self.atomic_system = atom
        if (bound_conf is None) and (scattering_conf is None):
            raise ValueError("At least one of bound_conf or scattering_conf should be passed")
    ...
    def run_dhfs(self) -> None:
        self.dhfs_handler = dhfs_handler(self.atomic_system, self.atomic_system.name)
        self.dhfs_handler.run_dhfs(self.bound_config.max_r,
                                   self.bound_config.n_radial_points,
                                   self.bound_config.radius_unit,
                                   iverbose=0)
    ...
    def find_bound_states(self):
        ...
        self.bound_handler.r_grid = self.dhfs_handler.rad_grid
        self.bound_handler.rv_grid = self.dhfs_handler.rv_modified
        self.bound_handler.find_bound_states()
        
    
    def find_scattering_states(self):
        # solve scattering states in final atom
        self.scattering_handler_final = scattering_handler(self.atomic_system.Z,
                                                           int(self.atomic_system.occ_values.sum()),
                                                           self.scattering_config)
        self.scattering_config.print()
        self.scattering_handler_final.set_potential(self.dhfs_handler.rad_grid, self.dhfs_handler.rv_modified)
        
        self.scattering_handler_final.compute_scattering_states()
