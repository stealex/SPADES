...
C
      PO=PT(ILAST)
      QO=QT(ILAST)
      PHASE=ATAN2(PO*QA(ILAST)-QO*PA(ILAST),QO*PB(ILAST)-PO*QB(ILAST))
...
C
      ...
      SD=SIN(PHASE)
      IF(ABS(PO).GT.EPS) THEN
        RNORM=(CD*PA(ILAST)+SD*PB(ILAST))/PO
      ELSE
        RNORM=(CD*QA(ILAST)+SD*QB(ILAST))/QO
      ENDIF
C
      DO I=1,ILAST
        PT(I)=RNORM*PT(I)
...
C
      ...
      ZQIB=ZQB(ILAST)
C
      ZPHASE=(ZPO*(ZQIA+ZI*ZQIB)-ZQO*(ZPIA+ZI*ZPIB))
     1      /(ZQO*(ZPIA-ZI*ZPIB)-ZPO*(ZQIA-ZI*ZQIB))
C  ****  The real phase shift is reduced to the interval (-PI/2,PI/2).
      ZPH=-ZI*CDLOG(ZPHASE)*0.5D0
      PHASER=ZPH
      PHASEI=-ZI*ZPH
